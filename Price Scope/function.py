...
def fetch_product_categories(connection):
    ...
    query = "SELECT DISTINCT categorie_Rayon FROM `articles`"
...
def fetch_products_by_category(connection, category):
    ...
    query = f"SELECT DISTINCT nom_Article FROM `articles` WHERE categorie_Rayon = '{category}'"
...
def fetch_product_price(connection, product_name, start_date, end_date):
    ...
    try:
        ...
        """
        price_df = pd.read_sql(query, connection)
        return float(price_df['prix_moyen'].iloc[0]) if not price_df.empty else None
